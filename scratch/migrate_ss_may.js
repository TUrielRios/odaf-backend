...
const appointments = JSON.parse(fs.readFileSync(path.join(__dirname, 'ss_may_appointments.json'), 'utf8'));

const PROFESSIONAL_ID = 13;
const SERVICE_ID = 2;
...
async function findOrCreatePatient(fullName) {
  ...
  // Generate a unique DNI placeholder (99401xxx) - Offset slightly from April to avoid rare overlap
  const count = await Paciente.count({ where: { numero_documento: { [Op.like]: '99401%' } } });
  const dni = (99401000 + count + 1).toString();
    ...
    for (const slot of slots) {
      try {
        ...
        const turno = await Turno.create({
          ...
          hora_fin: endTime,
          estado: STATUS,
          notas: `Migrado de sistema anterior (SS Mayo). Paciente original: ${slot.patientName}`,
          sobre_turno: true
        });
