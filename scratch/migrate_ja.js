...
const appointments = JSON.parse(fs.readFileSync(path.join(__dirname, 'ja_appointments.json'), 'utf8'));

const PROFESSIONAL_ID = 8;
const SERVICE_ID = 7;
...
async function findOrCreatePatient(fullName) {
  ...
  // Generate a unique DNI placeholder (992xxxxx)
  const count = await Paciente.count({ where: { numero_documento: { [Op.like]: '992%' } } });
  const dni = (99200000 + count + 1).toString();
    ...
    for (const slot of slots) {
      try {
        ...
        const turno = await Turno.create({
          ...
          hora_fin: endTime,
          estado: STATUS,
          notas: `Migrado de sistema anterior (JA). Paciente original: ${slot.patientName}`,
          sobre_turno: true
        });
