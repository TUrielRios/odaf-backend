...
const appointments = JSON.parse(fs.readFileSync(path.join(__dirname, 'ja_may_remaining_appointments.json'), 'utf8'));
...
async function findOrCreatePatient(fullName) {
  ...
  // Generate a unique DNI placeholder (99201xxx) - Offset from previous JA ranges
  const count = await Paciente.count({ where: { numero_documento: { [Op.like]: '99201%' } } });
  const dni = (99201000 + count + 1).toString();
    ...
    for (const slot of slots) {
      try {
        ...
        const turno = await Turno.create({
          ...
          hora_fin: endTime,
          estado: STATUS,
          notas: `Migrado de sistema anterior (JA Mayo Parte 2). Paciente original: ${slot.patientName}`,
          sobre_turno: true
        });
