...
const appointments = JSON.parse(fs.readFileSync(path.join(__dirname, 'mr_appointments.json'), 'utf8'));

const PROFESSIONAL_ID = 17;
const SERVICE_ID = 7;
...
async function findOrCreatePatient(fullName) {
  ...
  // Generate a unique DNI placeholder (993xxxxx)
  const count = await Paciente.count({ where: { numero_documento: { [Op.like]: '993%' } } });
  const dni = (99300000 + count + 1).toString();
...
async function run() {
  // Ensure link exists
  await ProfesionalServicio.findOrCreate({
    where: { profesional_id: PROFESSIONAL_ID, servicio_id: SERVICE_ID }
  });
    ...
    for (const slot of slots) {
      try {
        ...
        const turno = await Turno.create({
          ...
          hora_fin: endTime,
          estado: STATUS,
          notas: `Migrado de sistema anterior (MR). Paciente original: ${slot.patientName}`,
          sobre_turno: true
        });
