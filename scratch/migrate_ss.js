...
const appointments = JSON.parse(fs.readFileSync(path.join(__dirname, 'ss_appointments.json'), 'utf8'));
...
async function findOrCreatePatient(fullName) {
  ...
  // Generate a unique DNI placeholder (994xxxxx)
  const count = await Paciente.count({ where: { numero_documento: { [Op.like]: '994%' } } });
  const dni = (99400000 + count + 1).toString();
    ...
    for (const slot of slots) {
      try {
        ...
        const turno = await Turno.create({
          ...
          hora_fin: endTime,
          estado: STATUS,
          notas: `Migrado de sistema anterior (SS). Paciente original: ${slot.patientName}`,
          sobre_turno: true
        });
