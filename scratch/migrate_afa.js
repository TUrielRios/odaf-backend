const { Turno, Paciente, Prestacion, Profesional, Servicio, SubServicio } = require('../src/models');
const { Op } = require('sequelize');
const fs = require('fs');
const path = require('path');

const appointments = JSON.parse(fs.readFileSync(path.join(__dirname, 'afa_appointments.json'), 'utf8'));
...
async function findOrCreatePatient(fullName) {
  ...
  // Generate a unique DNI placeholder (991xxxxx)
  const count = await Paciente.count({ where: { numero_documento: { [Op.like]: '991%' } } });
  const dni = (99100000 + count + 1).toString();
    ...
    for (const slot of slots) {
      try {
        ...
        const turno = await Turno.create({
          ...
          hora_fin: endTime,
          estado: STATUS,
          notas: `Migrado de sistema anterior (AFA). Paciente original: ${slot.patientName}`,
          sobre_turno: true // Enable to avoid overlap conflicts
        });
